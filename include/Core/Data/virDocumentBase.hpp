...
namespace core
{
	...
	class CORE_API TransactionGuard
	{
	public:
		TransactionGuard(DocumentBase* doc);
		~TransactionGuard();
	...
	class CORE_API DocumentBase : public NodeSetBase
	{
	public:
		DocumentBase();
		...
		//void insertCommitCallback(std::function<void()> callback);
		bool commit();
		virtual bool run() = 0;
	...
	private:
		...
		bool mIsExecuting;															// 标记当前是否正在执行 commit，避免重入
		...
		//std::vector<std::function<void()>> mCommitCallbacks;						// 事务提交后的回调列表，允许外部注册在 commit 后执行的函数
	};
}