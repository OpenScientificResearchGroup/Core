...
namespace core
{
	...
	bool NodeSetBase::read(const nlohmann::json& j)
	{
		...
		nlohmann::json nodeJ = j["node"];
		for (const auto& node : nodeJ)
		{
			auto& srvMgr = ServiceManager::get();
			std::unique_ptr<NodeBase> newNode;
			auto cmdPtr = srvMgr.getNamedService<std::function<std::unique_ptr<core::NodeBase>()>>(node["type"].get<std::string>());
			if (cmdPtr) newNode = (*cmdPtr)();
			newNode->read(node);
			mNodes[newNode->getUuid()] = std::move(newNode);
		}
		nlohmann::json nodeSetJ = j["node_set"];
		for (const auto& nodeSet : nodeSetJ)
		{
			auto& srvMgr = ServiceManager::get();
			std::unique_ptr<NodeSetBase> newNodeSet;
			auto cmdPtr = srvMgr.getNamedService<std::function<std::unique_ptr<core::NodeSetBase>()>>(nodeSet["type"].get<std::string>());
			if (cmdPtr) newNodeSet = (*cmdPtr)();
