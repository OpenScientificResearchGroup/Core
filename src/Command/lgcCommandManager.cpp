...
namespace core
{
	CommandManager::CommandManager(size_t maxHistory)
	...
	void CommandManager::submit(std::unique_ptr<CommandBase> cmd)
	{
		...
		try {
			...
			// 这里的逻辑取决于你的架构设计，通常 submit 代表“执行并记录”
	...
	void CommandManager::beginTransaction(const std::string& label)
	{
		if (mIsInTransaction) return; // 不支持嵌套事务（或者你可以实现栈式嵌套）
		mIsInTransaction = true;
		mPendingTransaction = std::make_unique<CompositeCommand>(label);
	}

	void CommandManager::endTransaction()
	{
		if (!mIsInTransaction) return;
		...
		if (mPendingTransaction && !mPendingTransaction->isEmpty())
		{
			...
			// 你可能需要调整 CompositeCommand::execute 的逻辑避免重复执行
	...
	void CommandManager::trimHistory()
	{
		...
		while (mHistory.size() > (size_t)(mCurrentIndex + 1))
		{
			mHistory.pop_back();
		}
		...
		if (mSavePointIndex > mCurrentIndex) {
			mSavePointIndex = -2; // 无效值
		}
	}
	...
	std::string CommandManager::getUndoLabel() const {
		if (allowUndo()) return mHistory[mCurrentIndex]->getLabel();
		return "";
	}

	std::string CommandManager::getRedoLabel() const {
		if (allowRedo()) return mHistory[mCurrentIndex + 1]->getLabel();
		return "";
	}
